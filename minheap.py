...
class MinHeap:
	...
	def minHeapify(self, pos):
		...
		if not self.isLeaf(pos):
			if (self.Heap[pos].rideCost > self.Heap[self.leftChild(pos)].rideCost or
			self.Heap[pos].rideCost > self.Heap[self.rightChild(pos)].rideCost):

				# Swap with the left child and heapify
				# the left child
				if self.Heap[self.leftChild(pos)].rideCost < self.Heap[self.rightChild(pos)].rideCost:
	...
	def Print(self):
		for i in range(1, (self.size//2)+1):
			print(" PARENT : "+ str(self.Heap[i].rideCost)+" LEFT CHILD : "+
								str(self.Heap[2 * i].rideCost)+" RIGHT CHILD : "+
